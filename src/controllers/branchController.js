...
export const patchBranchCustomization = async (req, res) => {
  try {
    ...
    const before = {
      isClassicMenu: branch?.customization?.isClassicMenu === true,
      isClassicMenuwithFullImage:
        branch?.customization?.isClassicMenuwithFullImage === true,
    };

    // ✅ Require at least one supported key
    const canPatchClassic = hasOwn(c, "isClassicMenu");
    const canPatchPhoto = hasOwn(c, "isClassicMenuwithFullImage");

    if (!canPatchClassic && !canPatchPhoto) {
      return res.status(400).json({
        message:
          "customization.isClassicMenu or customization.isClassicMenuwithFullImage is required",
        buildTag: "customization-route-v3",
      });
    }

    // ✅ Apply even if value is false
    if (canPatchClassic) {
      branch.set("customization.isClassicMenu", toBool(c.isClassicMenu));
    }

    if (canPatchPhoto) {
      branch.set(
        "customization.isClassicMenuwithFullImage",
        toBool(c.isClassicMenuwithFullImage),
      );
    }
    ...
    const after = {
      isClassicMenu: fresh?.customization?.isClassicMenu === true,
      isClassicMenuwithFullImage:
        fresh?.customization?.isClassicMenuwithFullImage === true,
    };
